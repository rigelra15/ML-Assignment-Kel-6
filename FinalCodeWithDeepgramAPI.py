...
def find_word_timestamps(transcript, target_word):
    ...
    for item in transcript["results"]["channels"][0]["alternatives"][0]["words"]:
        if item["word"].lower() == target_word.lower():
            start_offset = max(0, item["start"] - 0.07)  # Offset 0.5 detik sebelum kata
            end_offset = item["end"] + 0.02  # Offset 0.5 detik setelah kata
            word_timestamps.append((start_offset, end_offset))
    return word_timestamps

# Fungsi untuk memotong audio berdasarkan waktu mulai dan akhir
def cut_audio(audio_file_path, word_timestamps):
    ...
    for start, end in word_timestamps:
        ...
        end_ms = end * 1000
        extracted_segments.append(audio[start_ms:end_ms])
...
def main():
    ...
    for audio_file_name in sorted_audio_files:
        ...
        if word_timestamps:
            # Potong audio berdasarkan waktu mulai dan akhir dengan offset
